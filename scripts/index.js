...
если не менялись поля. Это произошло после того, как я убрал enableValidation(validationObj) из 130 строки, в глобальную область, при начале загрузки страницы. Сейчас, получается, проверка происходит пока поля еще не заполнены. Пока не понимаю , как в этом случае правильно поступить.
